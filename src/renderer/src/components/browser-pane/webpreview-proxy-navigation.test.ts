...
describe('navigateProxyIframe', () => {
  it('assigns the typed src when it differs from the current attribute', () => {
    ...
    ifr.setAttribute('src', '/__orca/webpreview/s1/old')
    navigateProxyIframe(ifr, '/__orca/webpreview/s1/new')
    expect(ifr.getAttribute('src')).toBe('/__orca/webpreview/s1/new')
  })
  ...
  it('force-reloads via about:blank when the src attribute already matches', () => {
    ...
    const ifr = document.createElement('iframe')
    const target = '/__orca/webpreview/s1/page'
