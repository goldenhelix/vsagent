...
export function createExternalWatchEventHandler(
  ...
  const handleFsChanged = (payload: FsChangedPayload): void => {
    ...
    const dbg =
      ...
      window.localStorage?.getItem('VSAGENT_EDITOR_SYNC_DEBUG') === '1'
    if (dbg) {
      console.log(
        ...
        payload.events
          .slice(0, 3)
          .map((e) => `${e.kind}:${'absolutePath' in e ? e.absolutePath : '?'}`)
      )
    }
    if (!target) {
      return
    ...
    for (const relativePath of changedFiles) {
      ...
      if (matching.length === 0) {
        if (dbg) {
          console.log('[editor-sync] skip', relativePath, 'no matching open file')
        }
        continue
      }
      if (matching.some((f) => f.isDirty)) {
        if (dbg) {
          console.log('[editor-sync] skip', relativePath, 'tab is dirty')
        }
        continue
      }
      ...
      if (recentSelfWrite) {
        if (dbg) {
          console.log('[editor-sync] self-write-aware reload', relativePath, 'within TTL')
        }
      ...
      if (dbg) {
        console.log(
          '[editor-sync] reload',
          relativePath,
          `modes=${matching.map((m) => m.mode).join(',')}`
        )
      }
      scheduleDebouncedExternalReload(notification)
    }
