...
function createWebPreloadApi(): Partial<PreloadApi> {
  return {
    app: {
      ...
      // Why: feature-gate accessor used by isWebMode() and friends. In the web
      // bridge we are by definition the web flavor on whatever platform the
      // server is hosted on; the renderer-side `loadRuntimeFlavor()` sets the
      // identity from this call.
      getRuntimeFlavor: () => Promise.resolve({ web: true, platform: 'web' })
    },
    e2e: {
    ...
    session: {
      ...
      set: async ({ state }) => {
        writeJson(SESSION_STORAGE_KEY, sanitizeWebRuntimeWorkspaceSession(state))
      },
      setSync: ({ state }) => {
        writeJson(SESSION_STORAGE_KEY, sanitizeWebRuntimeWorkspaceSession(state))
      },
      // Why: web mode delivers cross-client session sync via the web-gateway's
      // remoteWorkspace channel, not the IPC `session.onChanged` event. Stub
      // the listener so the type contract is satisfied and Settings/etc. that
      // subscribe to it just don't get desktop-side updates.
      onChanged: () => noopUnsubscribe
    },
    onboarding: {
...
function createFileApi(): NonNullable<Partial<PreloadApi>['fs']> {
  return {
    // Why: web mode has no native file dialog; these surface in panes that are
    // gated to !isWebMode() in the renderer but the type contract still
    // requires them. Stubs return safely-shaped empty results.
    autocompleteDir: async (input: string) => ({
      parent: input,
      suggestions: [],
      inputIsExistingDir: false,
      inputAbsolute: input,
      inputExists: false
    }),
    browseDir: async ({ dirPath }) => ({ resolvedPath: dirPath, entries: [] }),
