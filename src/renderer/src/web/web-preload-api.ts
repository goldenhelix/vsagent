...
// Why: captured at module load — the pairing flow strips query+hash from the
// URL (history.replaceState) before installWebPreloadApi runs.
const EXPOSE_STORE_REQUESTED =
...
export function installWebPreloadApi(): void {
  ...
  // Why (VSAgent fork): the store module's own expose gate reads window.api
  // before it exists in the web client, so honor ?orcaExposeStore=1 here for
  // Playwright smoke tests (store is reachable via React DevTools regardless).
  if (EXPOSE_STORE_REQUESTED) {
    void import('../store').then((storeModule) => {
      ;(window as unknown as Record<string, unknown>).__store = storeModule.useAppStore
    })
  }
}
...
function createWebPreloadApi(): Partial<PreloadApi> {
  ...
  return {
    ...
    e2e: {
      // Why (VSAgent fork): ?orcaExposeStore=1 lets Playwright smoke tests and
      // remote debugging read the Zustand store, mirroring VITE_EXPOSE_STORE on
      // desktop (the store is already reachable via React DevTools).
      getConfig: () => createE2EConfig({ exposeStore: EXPOSE_STORE_REQUESTED })
    },
    settings: {
...
// Why (VSAgent fork): browser panes in the web client are iframe-backed by the
// server-side webpreview proxy; sessions are minted over the paired RPC so the
// unguessable proxy path is the only capability the iframe needs.
function createWebPreviewApi(): NonNullable<PreloadApi['webPreview']> {
  type SessionResult = { session: { id: string; targetOrigin: string; proxyPath: string } | null }
  return {
    create: async (args) => {
      const result = await callRuntimeResult<SessionResult>('webpreview.create', args)
      if (!result.session) {
    ...
    setOrigin: async (args) => {
      const result = await callRuntimeResult<SessionResult>('webpreview.setOrigin', args)
      return result.session
    },
    delete: async (args) => {
