...
export function createStaticWebClientHandler(
  ...
  // Why (VSAgent fork): --serve-storage-namespace is normalized into this env
  // var at startup, so the handler needs no plumbing through the transport.
  const storageNamespace =
    (options.storageNamespace ?? process.env.VSAGENT_STORAGE_NAMESPACE)?.trim() || null
  return (request, response) => {
    void handleStaticRequest(resolvedRoot, request, response, storageNamespace)
  }
}

// Why (VSAgent fork): multiple VSAgent apps can share one browser origin under
// sub-URL routes; the web client scopes its localStorage by this namespace
