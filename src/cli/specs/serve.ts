...
export const SERVE_COMMAND_SPECS: CommandSpec[] = [
  {
    ...
    usage:
      'orca serve [--port <port>] [--host <ip>] [--pairing-address <host>] [--mobile-pairing] [--no-pairing] [--project-root <path>] [--recipe-json] [--json]',
    allowedFlags: [
      ...GLOBAL_FLAGS,
      'port',
      'host',
    ...
    notes: [
      ...
      'Use --host to bind a single interface (e.g. a Tailscale IP) instead of the default 0.0.0.0 (all interfaces). The pairing/web-client URL then advertises that host unless --pairing-address overrides it.',
